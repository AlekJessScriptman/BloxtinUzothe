...
-- New Feature: Auto Collect Chest
createLabel("💰 Auto Collect Chest")
createToggleButton("Auto Chest", function(active)
    if active then
        RunService.Heartbeat:Connect(function()
            for _, chest in pairs(workspace:GetDescendants()) do
                if chest.Name:lower():find("chest") and chest:IsA("Part") then
                    ...
                    if hrp then
                        TweenService:Create(hrp, TweenInfo.new(0.5), {CFrame = CFrame.new(chest.Position + Vector3.new(0, 3, 0))}):Play()
                    end
                end
            end
        end)
...
-- New Feature: Auto Accept Trade
createLabel("🤝 Auto Accept Trade")
createToggleButton("Accept Trades", function(active)
    if active then
        local success, err = pcall(function()
            ...
            RunService.RenderStepped:Connect(acceptTrade)
        end)
        if not success then warn("[Auto Accept Trade] Error:", err) end
    end
end)

-- New Feature: Join Player by Name
createLabel("🌍 Join Player by Name")
local joinBox = createTextBox("Player username")
createToggleButton("Join", function(active)
    if active then
        local username = joinBox.Text
        if username ~= "" then
            print("[Join Player] Attempting to join user:", username)
            -- In real case, use third-party API or TeleportService if server ID known
        end
    end
end)
