...
-- Features createLabel("❄️ Freeze Trade") createToggleButton("Freeze", function(active) if active then print("[Freeze Trade] Enabled") end end)

createLabel("⚔️ Auto Farm") local npcInput = createTextBox("Enter NPC Name") local farming = false createToggleButton("Auto Farm", function(active) farming = active end)

RunService.Heartbeat:Connect(function() if farming then local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") for _, npc in pairs(workspace:GetDescendants()) do if npc:IsA("Model") and npc.Name == npcInput.Text and npc:FindFirstChild("HumanoidRootPart") then local pos = npc.HumanoidRootPart.Position TweenService:Create(hrp, TweenInfo.new(1), {CFrame = CFrame.new(pos + Vector3.new(0, 3, 0))}):Play() end end end end)
...
createLabel("👣 Follow Player") local followBox = createTextBox("Player name") local following = false createToggleButton("Follow", function(active) following = active end)

RunService.Heartbeat:Connect(function() if following then local target = Players:FindFirstChild(followBox.Text) local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") and hrp then local pos = target.Character.HumanoidRootPart.Position hrp.CFrame = CFrame.new(hrp.Position, pos) end end end)
...
createLabel("🍀 Lucky Playtime for Gacha") createToggleButton("Lucky Timer", function(active) if active then print("[Lucky Playtime] You feel lucky! Maybe better drops?") end end)
